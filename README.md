...
~/.codex-tools/environments/python-tools/bin/pip install .
# Or, if you prefer uv and have it installed:
# uv sync
```
...
You can run the tool either through the helper scripts (recommended) or the
wrapper script / direct Python CLI.
...
```bash
./scripts/init_email.sh --config config/config.toml [-a user@domain]
```

Daily triage run:

```bash
./scripts/run_email.sh --config config/config.toml [-a user@domain] \\
...
### Option B: wrapper script (when installed via Codex tools)

If you have a top-level `run_email_categorise.sh` wrapper (for example under
`~/.codex-tools/bin/email_categorise`), you can continue to use:
...
```bash
python -m email_categorise run --config config/config.toml [-a user@domain] -v
```
